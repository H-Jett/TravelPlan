...
而不是从站点消失。**给新目的地补 `geo` 就能自动升级到真实底图**，不需要改代码。
（两个目的地现在都已迁移，没有降级中的了。）

区域划分与 `geo` 是两件事：`geo` 决定「画在哪」，**`regions` 决定「画多大」** ——
一个区域共用一张底图和一套缩放，所以跨度很远的点会被压到看不清。
成都的数据里原本既没有 `geo` 也没有 `regions`：9 个点挤在一个隐式区域里横跨 166km，
缩放下宽窄巷子↔人民公园 只差 6px。拆成 成都市区 / 都江堰·青城山 / 乐山·峨眉山
（跟随行程结构：Day1 市区、Day2 自驾、Day3-5 高铁+山岳）后为 25px。
拆区会让跨区的天在每个区域各显示一段，详见 `.claude/skills/travel-plan-site/SKILL.md`。
