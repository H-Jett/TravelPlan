...
function travelMapMarkup(source, route) {
  ...
  return `<div class="travel-map-block ${route ? "is-daily" : "is-overview"}" ${route ? `style="--route-color:${route.color}"` : ""}>
    ...
    <div class="map-utility"><span>${route ? "点圆点看地图 · 点图标看交通" : escapeHtml(mapNote)}</span>${source.projection&&source.projection.type==="web-mercator"?'<a class="map-attribution" href="https://www.openstreetmap.org/copyright" target="_blank" rel="noopener">© OpenStreetMap contributors</a>':''}<button type="button" data-expand-map="${id}">放大 ↗</button></div>
  </div>`;
}
