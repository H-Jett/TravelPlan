...
26. **住宿数据分三处，各管一件事 —— 别只改一处。**
    - `accommodations[]`（顶层）：**页面完全不渲染**，只作记录。`template/SKILL.md:131` 写明
      「页面上要显示的入住、退房和住宿文字仍写入对应的 `day.schedule[]`」。
    - `day.schedule[]` 里那几条 `check-in` / `transfer`：**这才是用户看到的**。
      酒店名放正文，地址与入住/退房时间放 `notes[]`；酒店名一般超过短标题 8 字上限，
      **不能塞进 `label`**（写「入住与退房」「位置」这种）。
    - `preTrip.todoItems[]`：会渲染进行前清单（`app.js:898`），订完就把对应条目标 `completed: true`。
    - 另有一类**不渲染、但公开页面上挂着就是假信息**的：`issuesAndUncertainties[]`、
      `bookingsAndTickets[].status`、`preTrip.missingNote`。订票/订房后要专门回去收口「待补充」——
      本轮就是漏了三条（住宿/航班/交通方式），审计时才翻出来。

    ⚠️ **订下住宿 = 换 base，不只是填个名字。** 本轮三家酒店**没有一家**在原行程假定的位置：
    济州原按「莲洞」（实在 제주시청 一带）、釜山原按「西面」（实在松岛）、
    首尔原按「首尔站附近」（实在 성신여대입구）。每加入一家酒店都要回头过一遍：

    1. **前后接驳的正文与时刻** —— 含从酒店出发那天的首项、以及去机场/车站那天的末段；
       原按旧 base 写的打车金额、公交线路（如「机场 3001 路到莲洞」）会整条失效。
    2. **城市锚点地点**：`places[]` / `map.places[]` / `routeMap` 三处各有一个「代表点」被日程与
       地图引用（如 `p-jeju-city` 叫「济州市区（莲洞）」、`p-busan-city` 叫「釜山西面（市区）」）。
       锚点不跟着走，就会出现日程写「酒店 → …」而地图上钉着旧地方。
       但**「莲洞」「西面」若还出现在具体景点名里（Olive Young 莲洞店）就留着**，只改锚点。
    3. `days[].locations` 里对应的城市名。

    ⚠️ **路线时长核不到，就只写走法，不写分钟数，也不要靠挪时刻来掩盖。**
    本轮「松岛 → 青沙浦」查不到可直接引用的总时长，于是正文只写
    「先坐 3006 路到海云台一带再转村巴」——不编分钟数（事实纪律）。

    ⚠️ **锚点坐标取 OSM 实名地物（Nominatim 核），别拿门牌号硬凑。** 门牌地址常查不到、
    或落到邻街（本轮 `등대로 9` 查到的是离酒店 1.2 km 的另一段）。用旁边一个**有名字的地物**
    （제주시청 / 송도스카이워크 / 송림공원）并在数据里说明锚点取在哪，比假精确到门牌诚实。
    改完 `map.places[].geo` 后：`npm run build` 会**重算 `routeMap`**
    （所以**直接手改 `routeMap` 会被 build 覆盖，白改**），该区域底图 png 也会重建（取景框随点位变），
    然后跑 `npm run verify:projection` 确认投影没退化（`routeMap` 的点位是投影到画布像素的）。
